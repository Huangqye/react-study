...
function getAllDays(date: Dayjs) {
  const daysInMonth = date.daysInMonth(); // 29 ✅
  const startDate = date.startOf("month"); // ✅
  const day = startDate.day(); // 周四 ✅

  const daysInfo: Array<{ date: Dayjs; currentMonth: boolean }> = new Array(
    6 * 7
  ); // ✅
  for (let i = 0; i < day; i++) {
    // subtract(减去) 返回减去一定时间的对象
    daysInfo[i] = {
...
function MonthCalendar(props: MonthCalendarProps) {
  ...
  const { dateRender, dateInnerContent, value, selectHandle, curMonth } = props;
